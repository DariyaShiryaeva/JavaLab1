...
5. **Метод возвращает long-число с проверкой диапазона и формата** - `getLong(Scanner scanner)`

6. **Метод запрашивает выбор из двух вариантов (1 или 2)** - `getNumb12(Scanner scanner)`

7. **Метод заполняет массив целыми числами** - `getIntArray(Scanner scanner, int amount)`

8. **Метод выводит массив в форматированном виде** - `outArray(int[] arr, int amount)`
