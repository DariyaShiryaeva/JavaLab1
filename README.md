...
3. **Преобразование символа в соответствующее число** - `charToNum(char x)`
...
4. **Проверка двузначности числа** - `is2Digits(int x)`
...
5. **Проверка, вхождения числа num в промежуток [a,b], отношение a и b заранее неизвестно** - `isInRange(int a, int b, int num)`
...
6. **Проверка равенства трех введенных чисел** - `isEqual(int a, int b, int c)`
  Проверяем, что a == b
  И что a == c
...
7. **Возврат модуля числа** - `abs(int x)`
...
8. **Проверка делимости число либо на 5, либо на 3, не одновременно** - `is35(int x)`
...
9. **Поиск максимального из трех чисел** - `max3(int x, int y, int z)`
...
10. **Нахождение сумы двух чисел и возврат числа 20, если сумма варьируется между 10 и 19 включительно** - `sum2(int x, int y)`
  Вычисляем сумму x + y
...
11. **Определение дня недели** - `day(int x)`
...
12. **Возврат строки, в которой будут записаны все числа от 0 до x** - `listNums(int x)`
...
3.  **Метод проверяет, является ли строка числом с плавающей точкой** - `isDouble(String str)`

4. **Метод проверяет корректность long-числа с дополнительной валидацией** - `isValidLong(String str)`

5. **Метод гарантированно возвращает целое число с обработкой ошибок** - `getInteger(Scanner scanner)`

6. **Метод возвращает long-число с проверкой диапазона и формата** - `getLong(Scanner scanner)`

7. **Метод запрашивает выбор из двух вариантов (1 или 2)** - `getNumb12(Scanner scanner)`

8. **Метод заполняет массив целыми числами** - `getIntArray(Scanner scanner, int amount)`

9. **Метод выводит массив в форматированном виде** - `outArray(int[] arr, int amount)`
